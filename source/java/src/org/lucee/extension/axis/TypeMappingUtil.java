...
public class TypeMappingUtil {
	...
	public static void registerDefaults(TypeMappingRegistry tmr) {
		...
		if (!tm.isRegistered(QueryBean.class, RPCConstants.QUERY_QNAME)) tm.register(QueryBean.class, RPCConstants.QUERY_QNAME,
				new BeanSerializerFactory(QueryBean.class, RPCConstants.QUERY_QNAME), new BeanDeserializerFactory(QueryBean.class, RPCConstants.QUERY_QNAME));

		// Adding custom string serialization for non printable characters.
		tm.register(String.class, RPCConstants.STRING_QNAME, new StringSerializerFactory(String.class, RPCConstants.STRING_QNAME),
	...
	public static void registerBeanTypeMapping(javax.xml.rpc.encoding.TypeMapping tm, Class clazz, QName qName) {
		...
		if (clazz.isArray()) {
			QName ct = Axis1Caster.toComponentType(qName, null);
			if (ct != null) {
				tm.register(clazz, qName, new ArraySerializerFactory(clazz, ct), new ArrayDeserializerFactory(ct));
				return;
			}
		}
