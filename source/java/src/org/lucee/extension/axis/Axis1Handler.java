...
public class Axis1Handler implements WSHandler {
	...
	public TypeEntry getContainedElement(TypeEntry type, String name, TypeEntry defaultValue) {
		...
		if (v != null) {
			...
			while (it.hasNext()) {
				...
				tmp = engine.getListUtil().last(ed.getQName().getLocalPart(), ">", true);

				if (tmp.equalsIgnoreCase(name)) return ed.getType();
			}
		}
		return defaultValue;
	}
	 ...
	 * { if (client != null) { Call call = (Call) ((Axis1Client)client).getLastCall(); if (call == null)
	 * throw new AxisFault("web service was not invoked yet"); return call.getMessageContext(); }
	 * MessageContext context = MessageContext.getCurrentContext(); if (context == null) throw new
	 * AxisFault("not inside a Soap Request"); return context; }
	 */
