...
public class BeanDeserializer extends DeserializerImpl implements Serializable {
	 ...
	 * startElement
	 * 
	 * The ONLY reason that this method is overridden is so that the object value can be set or a
	 * reasonable exception is thrown indicating that the object cannot be created. This is done at this
	 * point so that it occurs BEFORE href/id processing.
	 * 
	 ...
	 * @return is a Deserializer to use to deserialize a child (must be a derived class of SOAPHandler)
	 *         or null if no deserialization should be performed.
	 */
	@Override
	...
	/**
	 * Get a BeanPropertyDescriptor which indicates where we should put extensibility elements (i.e. XML
	 * which falls under the auspices of an &lt;xsd:any&gt; declaration in the schema)
	 *
	...
	public void onStartElement(String namespace, String localName, String prefix, Attributes attributes, DeserializationContext context) throws SAXException {
		...
		for (int i = 0; i < attributes.getLength(); i++) {
			...
			if (bpd != null) {
				...
				if (dSer == null) throw new SAXException(Messages.getMessage("unregistered00", bpd.getType().toString()));

				if (!(dSer instanceof SimpleDeserializer)) throw new SAXException(Messages.getMessage("AttrNotSimpleType00", bpd.getName(), bpd.getType().toString()));
	 ...
	 * @param href String is the value of the href attribute, which is used to determine whether the
	 *            child element is complete or an href to another element.
