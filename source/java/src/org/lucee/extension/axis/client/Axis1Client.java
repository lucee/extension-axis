...
public final class Axis1Client implements WSClient {
	...
	private Object _call(PageContext pc,Config secondChanceConfig,String methodName, Struct namedArguments,Object[] arguments) throws PageException, ServiceException, RemoteException {
        ...
        if(arguments!=null) {
            ...
            if(inNames.size() != namedArguments.size())
                throw exception.createExpressionException("Invalid arguments count for operation " + methodName+" ("+namedArguments.size()+" instead of "+inNames.size()+")");
	...
	private ClassLoader getClassLoader(PageContext pc, Config secondChanceConfig) {
		...
		catch (Exception e) {e.printStackTrace();/* MUST report the exception or throw it */}
		return cl;
	}
	...
	private String getErrorDetailForArguments(String[] names, String[] argKeys) {
        ...
        for(int i=0;i<names.length;i++) {
            ...
            if(!found) {
                if(names.length>1)
                    return "missing argument with name ["+name+"], needed argument are ["+_engine.getListUtil().toList(names,", ")+"]";
	...
	private static SOAPHeaderElement toSOAPHeaderElement(String namespace,
		...
		if (el != null)
			return new org.apache.axis.message.SOAPHeaderElement(el);
	...
	private static Object toValue(org.apache.axis.message.SOAPHeaderElement header, boolean asXML)
			throws Exception {
		if (header == null)
			return "";
		if (asXML) {
